    ...
    <tbody>
        ...
        if (isset($_POST["search_food"])) {
            ...
            if ($result_food->num_rows > 0) {
                while ($row = $result_food->fetch_assoc()) { ?>
                    <tr>
                        ...
                        if (isset($_SESSION["account"])) { ?>
                            <td class="align-middle">
                                <a href="CRUD/update.php?food_id=<?php echo $row['food_id']; ?>">Sửa</a>
                            </td>
                            <td class="align-middle">
                                <a onclick="return Del('<?php echo $row['food_name']; ?>')" href="CRUD/delete.php?food_id=<?php echo $row['food_id']; ?>">Xóa</a>
                            </td>
                        <?php } else { ?>
                            <td class="align-middle">
                ...
                <tr>
                    <td colspan="8"><?php echo "<b>Không có món ăn phù hợp.</b>"; ?></td>
                </tr>
        <?php }
        }
        $conn->close();
...
<?php if (isset($_SESSION["account"])) { ?>
    <a class="btn btn-primary" href="CRUD/insert.php">Thêm món ăn</a>
<?php }
